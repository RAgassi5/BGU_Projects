...
Room::Room(const std::string &RoomID, int fireCamp, int monsterCurrentHealth, int monsterDamage) {
    this->RoomID = RoomID;
    ...
    this->roomCount = 0;
    this->MonsterInDaRoom = Entity("Monster", monsterCurrentHealth, monsterDamage);
}
...
Room::Room(const Room &OtherRoom) {
    ...
    if (OtherRoom.roomList != nullptr && OtherRoom.roomCount > 0) {
        ...
        for (int i = 0; i < OtherRoom.roomCount; i++) {
            if(OtherRoom.roomList[i] != nullptr) {
                this->roomList[i] = new Room(*OtherRoom.roomList[i]);
            }
            else {
    ...
    this->MonsterInDaRoom = OtherRoom.MonsterInDaRoom;
}
...
void Room::addRoomByIndex(Room *OtherRoom) {
    ...
    if (this->roomList == nullptr) {
        ...
        if (room_to_add_number < this->roomCount) {
            ...
            Room **NewList = new Room *[room_to_add_number + 1];
            for (int i = 0; i < room_to_add_number; i++) {
                NewList[i] = nullptr;
            }
            for (int i = 0; i < this->roomCount; i++) {
...
Entity *Room::get_MonsterInDaRoom() {
    if (this->monsterCurrentHealth == 0) {
        return nullptr;
    }
    return &this->MonsterInDaRoom;
}
